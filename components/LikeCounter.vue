<template>
  <div class="flex justify-center flex-col">
    <div class="flex items-center mt-4 text-gray-500">
      ...
      <p v-else-if="$fetchState.error" class="ml-1">
        Likes unaccessible
      </p>
    </div>

    <button type="button" class="w-20 inline-flex px-3.5 py-2 border border-transparent text-sm leading-4 font-medium rounded-full shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" @click="addLikesOnInterval">
      ...
      {{ initialLikes }} <span v-if="likesMaxed">max likes</span>
    </button>
  </div>
</template>
